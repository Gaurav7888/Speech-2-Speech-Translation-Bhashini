...
class LiveWav2Vec2:
    ...
    def list_microphones(pyaudio_instance):
        ...
        for i in range(0, numdevices):
            device_info = pyaudio_instance.get_device_info_by_host_api_device_index(0, i)
            name = device_info.get('name')
            max_input_channels = device_info.get('maxInputChannels')
            result += [[i, name, max_input_channels]]

        # Print available devices
        print("Available devices:")
        for device_id, name, max_channels in result:
            print(f"ID: {device_id}, Name: {name}, Max Input Channels: {max_channels}")
...
if __name__ == "__main__":
    ...
    asr = LiveWav2Vec2("ai4bharat/indicwav2vec-hindi")
